...
namespace Infrastructure.Services
{
    public class ProcessServices : IProcessServices
    {
        ...
        public ProcessServices(IGenericRepository<Process> processRepo, ATSContext context)
        {
        ...
        {
            var processList = await ProcessTransasctionsVerifiedForAdd(TransactionDate, status, remarks, cvrefIds);

            if (processList ==null) return null;
            
            return await _processRepo.AddListAsync(processList);
            ...
            enumProcessingStatus status, string remarks, int[] cvrefids)
        {
            ...
            foreach(var id in cvrefids)
            {
                var currentdata = await _context.Processes.Where(x => x.CVRefId == id)
                    ...
                    .OrderByDescending(x => x.Status).Take(1).FirstOrDefaultAsync();
                if(currentdata==null) throw new Exception ("failed to get process ids - ProcessTransactionsVerifiedForAdd");
                if (currentdata.NextProcessingId != status) 
                {
